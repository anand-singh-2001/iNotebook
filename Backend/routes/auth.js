...
//Route 1:Create a user using POST"/api/auth/createUser": 1st endpoint. No loggin required.
router.post('/createUser', [
    ...
    async (req, res) => {
        ...
        //Check whether with the same email exists already:
        try {
            ...
            //Create a new user:
            user = await User.create({  //returns a promise, therefore await is used
                name: req.body.name,
...
router.post('/login', [
    ...
    async (req, res) => {
        ...
        try {
            ...
            if (!user) {
                return res.status(400).json({ success, error: 'Enter correct login credentials' })
            ...
            if (!passwordCompare) { //if user is not found i.e, credentials don't match
                return res.status(400).json({ success, error: 'Enter correct login credentials' })
            }
...
router.post('/getuser', fetchuser, async (req, res) => {  //fetchuser is a middleware that is used/runs during login. The middleware runs before the async function.
    try {
        userId = req.user.id;
        console.log(userId);
