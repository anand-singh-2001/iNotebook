...
router.get("/fetchnotes", fetchuser, async (req, res) => {
  //fetchuser middleware is used as login is required here.
  try {
...
router.put("/updatenote/:id", fetchuser, async (req, res) => {
  //fetchuser middleware is used as login is required here./:id is used to specify that only the user with the respective id can update their note.
  try {
    const { title, description, tag } = req.body;
    ...
    } //Update the different fields of note if entered/available.
    if (tag) {
      newnote.tag = tag;
    }
    ...
    if (note.user.toString() !== req.user.id) {
      //.toSting() gives the id of the note that is being accessed, here if the note's id is not equal to the id of the user logged in.
...
router.delete("/deletenote/:id", fetchuser, async (req, res) => {
  //fetchuser middleware is used as login is required here./:id is used to aceept the id of the note.
  try {
