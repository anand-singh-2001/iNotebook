...
const NoteState = (props) => {
  ...
  const getNotes = async () => {
    ...
    const response = await fetch(`${host}/api/notes/fetchnotes`, {
      //fetching all the notes for the user.
      method: "GET",
      headers: {
  ...
  const editNote = async (id, title, description, tag) => {
    ...
    for (let index = 0; index < notes.length; index++) {
      ...
      if (element._id === id) {
        //checking if the current id matches with any of the stored item ids.
  ...
  return (
    ...
    <NoteContext.Provider
      ...
      {" "}
      {/**Anything wrapped inside the context will automatically be is passed to all the child classes */}
      {props.children}
    </NoteContext.Provider>
  );
};
